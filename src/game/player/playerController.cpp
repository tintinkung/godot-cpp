    ...
    void PlayerController::process_rotation_input(godot::Input* const input, double delta_time)
    {
        switch (this->get_input_mode(input))
        {
            ...
            case InputMode::Controller:
            {
                ...
                {
                    godot::UtilityFunctions::print("InputMode = Controller, but no controllers detected");
                }
                else
                {
                    godot::Vector2 target_rotation{ input->get_vector(
                        "rotate_left", "rotate_right", "rotate_up", "rotate_down") };
                    if (!target_rotation.is_zero_approx())
                    {
                        m_rotation_angle
                            = godot::Vector2(0, 0).angle_to_point(target_rotation) + godot::Math::deg_to_rad(90.0);
                    }
                }
                break;
            }
